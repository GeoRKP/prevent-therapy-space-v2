...
export function LegalPage({ docKey }) {
  ...
  return (
    <>
      ...
      <section className="relative section-pad bg-surface">
        <div className="container relative z-10 max-w-3xl">
          <p className="text-xs text-paper/56 mb-10">{t(`${docKey}.updated`)}</p>

          <div className="space-y-10">
            {sections.map((section, i) => (
              <div key={i}>
                <h2 className="text-lg font-bold text-paper mb-3 tracking-tight">
                  {section.h}
                </h2>
                <div className="space-y-3">
                  {section.p.map((paragraph, j) => (
                    <p key={j} className="text-sm text-paper/72 leading-relaxed">
                      {paragraph}
                    </p>
                  ))}
                </div>
