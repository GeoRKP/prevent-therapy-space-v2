...
function formatWhen(startISO, locale) {
...
export async function sendBookingEmails({ startISO, name, email, phone, locale, cancelLink }) {
  ...
  const { dateStr, timeStr } = formatWhen(startISO, lang);
  const duration = s.minutes(bookingConfig.durationMinutes);
  ...
  const patient = send({
    ...
    html: renderEmail({
      ...
      note: s.confirm.note(bookingConfig.cancelNoticeHours),
    }),
  });

  const el = STRINGS.el;
  const gr = formatWhen(startISO, "el");
...
function reminderPayload({ startISO, name, email, locale, kind, cancelLink }) {
  const lang = locale === "en" ? "en" : "el";
  const s = STRINGS[lang];
  ...
  return {
    ...
    html: renderEmail({
      ...
      rows: [
        [s.labels.date, dateStr],
        [s.labels.time, timeStr],
        [s.labels.duration, s.minutes(bookingConfig.durationMinutes)],
      ],
      cta: tpl.cta && cancelLink ? { label: tpl.cta, url: cancelLink } : undefined,
      note: tpl.note(bookingConfig.cancelNoticeHours),
    }),
  };
}
...
export async function scheduleReminderEmails({ startISO, name, email, locale, cancelLink }) {
  ...
  if (hoursUntil > 25) {
    const id = await send({
      ...reminderPayload({ startISO, name, email, locale, kind: "24h", cancelLink }),
  ...
  if (hoursUntil > 2.5) {
    const id = await send({
      ...reminderPayload({ startISO, name, email, locale, kind: "2h" }),
