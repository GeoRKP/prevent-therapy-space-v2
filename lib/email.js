...
// φόρμα επικοινωνίας). Ορίζεται από το env CONTACT_EMAIL (Vercel + .env.local)·
// το fallback είναι ο λογαριασμός που χρησιμοποιήθηκε κατά την ανάπτυξη.
export const CONTACT_EMAIL = process.env.CONTACT_EMAIL || "konpatsakis@gmail.com";
...
// Escape για τιμές που προέρχονται από τον χρήστη (όνομα, τηλέφωνο, email) —
// ποτέ αυτούσιο HTML μέσα στα emails.
function esc(value) {
  return String(value ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
...
function renderEmail({ heading, intro, rows, cta, note, lang = "el" }) {
  const rowsHtml = (rows || [])
    .map(
      ([label, value]) => `
        <tr>
  ...
  // Preheader: το κείμενο προεπισκόπησης των email clients — αόρατο στο σώμα.
  const preheader = `<div style="display:none;max-height:0;overflow:hidden;opacity:0;">${intro}</div>`;

  // Table-based κουμπί: το background στο td ώστε να δουλεύει και στο Outlook,
  // όπου τα paddings των <a> αγνοούνται.
  const ctaHtml = cta
    ? `<tr><td style="padding:26px 32px 0;">
         <table role="presentation" cellpadding="0" cellspacing="0"><tr>
           <td style="background:${BRAND.green};border-radius:999px;">
             <a href="${cta.url}" style="display:inline-block;color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;padding:12px 26px;">${cta.label}</a>
           </td>
         </tr></table>
       </td></tr>`
    : "";
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f2f4f3;padding:32px 12px;">
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;">
        ...
        ${ctaHtml}
        ${noteHtml}
        <tr><td style="padding:28px 32px 30px;color:${BRAND.muted};font-size:12px;line-height:1.7;border-top:1px solid #eceeed;">
          ${BRAND.name} · Θεοτοκοπούλου 55, Πατήσια, Αθήνα<br>
          <a href="tel:+306972952263" style="color:${BRAND.green};text-decoration:none;">697 295 2263</a>
          &nbsp;·&nbsp;
          <a href="${siteUrl()}" style="color:${BRAND.green};text-decoration:none;">${siteUrl().replace(/^https?:\/\//, "")}</a>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
const STRINGS = {
  el: {
    ...
    reminder24: {
      ...
      note: () => `Αν τελικά δεν μπορείτε να έρθετε, παρακαλούμε ενημερώστε μας τηλεφωνικά όσο νωρίτερα γίνεται, ώστε η ώρα να ελευθερωθεί για κάποιον άλλον.`,
    },
    reminder2: {
    ...
    review: {
      subject: () => `Ευχαριστούμε για την επίσκεψή σας!`,
      heading: "Ευχαριστούμε που μας επισκεφθήκατε",
      intro: (name) => `Γεια σας ${name}, ελπίζουμε η σημερινή σας συνεδρία να σας άφησε καλύτερα. Αν μείνατε ικανοποιημένοι, μια κριτική στο Google θα μας βοηθούσε πολύ — και βοηθά κι άλλους ανθρώπους με πόνο να μας βρουν.`,
      cta: "Αφήστε μια κριτική στο Google",
      note: () => `Χρειάζεται μόλις ένα λεπτό. Αν κάτι δεν πήγε καλά, θα χαρούμε να το ακούσουμε πρώτα εμείς — απαντήστε απλώς σε αυτό το email.`,
    },
    ownerBooked: {
    ...
    ownerCancelled: {
      ...
      intro: "Ο ασθενής ακύρωσε το παρακάτω ραντεβού μέσω του site. Η καταχώρηση αφαιρέθηκε από το ημερολόγιό σας και η ώρα είναι ξανά διαθέσιμη.",
    },
  },
  en: {
    ...
    reminder24: {
      ...
      note: () => `If you can no longer make it, please let us know by phone as early as possible so the slot frees up for someone else.`,
    },
    reminder2: {
    ...
    review: {
      ...
      intro: (name) => `Hello ${name}, we hope today's session left you feeling better. If you were happy with your visit, a Google review would help us a lot — and it helps other people in pain find us.`,
      cta: "Leave a Google review",
      note: () => `It only takes a minute. If something wasn't right, we'd love to hear it from you first — just reply to this email.`,
    },
    ownerBooked: {
...
async function send({ to, subject, html, scheduledAt }) {
  ...
  const { data, error } = await resend.emails.send({
    ...
    subject,
    html,
    // Το Reply πάει στο inbox του θεραπευτή, όχι στον αποστολέα no-reply
...
export async function sendBookingEmails({ startISO, name, email, phone, locale, cancelLink }) {
  ...
  const patient = send({
    ...
    html: renderEmail({
      ...
      intro: s.confirm.intro(esc(name)),
      rows: [
  ...
  const owner = CONTACT_EMAIL
    ? send({
        ...
        html: renderEmail({
          ...
          rows: [
            [el.labels.name, esc(name)],
            [el.labels.phone, esc(phone)],
            [el.labels.email, esc(email)],
 ...
 */
export async function scheduleReminderEmails({ startISO, name, email, locale }) {
  const config = await getBookingConfig();
  ...
  // email επιβεβαίωσης και μια υπενθύμιση θα ήταν θόρυβος. Χωρίς κουμπί
  // ακύρωσης: φτάνει τη στιγμή που κλείνει το 24ωρο παράθυρο ακύρωσης.
  if (hoursUntil > 25) {
    const id = await send({
      ...reminderPayload({ startISO, name, email, locale, kind: "24h", config }),
  ...
  // «Ευχαριστούμε + κριτική Google» ~3 ώρες μετά το τέλος της συνεδρίας.
  // Το Resend προγραμματίζει έως 30 μέρες μπροστά — στο οριακό άκρο του
  // παραθύρου κρατήσεων απλώς παραλείπεται.
  const reviewAt = start + config.durationMinutes * 60_000 + 3 * HOUR;
  if (reviewAt - Date.now() < 29.5 * 24 * HOUR) {
    const lang = locale === "en" ? "en" : "el";
    const tpl = STRINGS[lang].review;
    const id = await send({
      to: email,
      subject: tpl.subject(),
      html: renderEmail({
        ...
        heading: tpl.heading,
        intro: tpl.intro(esc(name)),
        cta: { label: tpl.cta, url: googleReviews.url },
        note: tpl.note(),
      }),
      scheduledAt: new Date(reviewAt).toISOString(),
    });
    if (id) ids.reviewEmailId = id;
  }
...
export async function sendRescheduledEmail({ startISO, name, email, locale, cancelLink }) {
  ...
  await send({
    ...
    html: renderEmail({
      ...
      intro: s.rescheduled.intro(esc(name)),
      rows: [
        [s.labels.date, dateStr],
        [s.labels.time, timeStr],
        [s.labels.duration, s.minutes(config.durationMinutes)],
  ...
  const patient = email
    ? send({
        ...
        html: renderEmail({
          ...
          intro: s.cancelled.intro(esc(name)),
          rows: [
            [s.labels.date, dateStr],
            [s.labels.time, timeStr],
          ],
