// Υπολογισμός διαθέσιμων ραντεβού: ωράριο (data/booking.js) μείον
// κατειλημμένα διαστήματα από το Google Calendar. Server-side μόνο.
...
export function slotsForDay(dateStr) {
  const { workingHours, durationMinutes } = bookingConfig;
...
export async function getAvailability() {
  const { timeZone, durationMinutes, minNoticeHours, maxAdvanceDays } = bookingConfig;
  ...
  for (let day = firstDay; day <= lastDay; day = addDays(day, 1)) {
    const slots = slotsForDay(day).filter((time) => {
...
export async function assertSlotAvailable(dateStr, timeStr) {
  ...
  const validTime = slotsForDay(dateStr).includes(timeStr);
