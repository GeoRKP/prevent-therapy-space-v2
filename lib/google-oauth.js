...
// Χωρίς αυτά τα δύο η σύνδεση είναι άχρηστη: η Google εμφανίζει ξεχωριστό
// πλαίσιο για κάθε δικαίωμα και ο χρήστης μπορεί να τα αφήσει ατσέκαριστα.
export const REQUIRED_SCOPES = GOOGLE_SCOPES.slice(0, 2);

/** Ποια από τα απαραίτητα δικαιώματα λείπουν από ένα scope string της Google. */
export function missingScopes(scopeString) {
  const granted = new Set(String(scopeString || "").split(/\s+/));
...
/** Ανταλλαγή του authorization code με tokens → { refreshToken, email, scope }. */
