...
import { missingScopes } from "@/lib/google-oauth";
...
// Δένεται με το refresh token που το παρήγαγε: αν ο θεραπευτής συνδέσει άλλον
// λογαριασμό από το /admin, το παλιό access token παύει να χρησιμοποιείται.
let tokenCache = { accessToken: null, expiresAt: 0, refreshToken: null, scope: "" };

async function getAccessToken() {
  const conn = await getGoogleConnection();
  if (!conn) {
    throw new Error("Δεν υπάρχει σύνδεση Google Calendar — συνδέστε λογαριασμό από το /admin");
  }

  if (
    tokenCache.accessToken &&
    tokenCache.refreshToken === conn.refreshToken &&
...
/** Δοκιμή ότι η ενεργή σύνδεση δίνει access token ΜΕ δικαιώματα ημερολογίου (για το /admin). */
export async function probeGoogleAccess() {
  try {
    await getAccessToken();
    const missing = missingScopes(tokenCache.scope);
    return { ok: missing.length === 0, missingScopes: missing };
  } catch (err) {
