...
export default function NotFound() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-surface py-32">
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
        <span className="text-[28vw] lg:text-[18vw] font-bold text-paper/[0.025] leading-none">
          404
        </span>
      </div>
      ...
      <div className="container relative z-10 text-center">
        ...
        <RevealText delay={0.1}>
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-paper tracking-tight mb-4">
            {ready ? t("title") : "Page not found"}
          </h1>
        </RevealText>

        <RevealText delay={0.2}>
          <p className="text-paper/70 mb-10 text-base lg:text-lg max-w-xl mx-auto">
            {ready ? t("description") : ""}
          </p>
        </RevealText>
...
function NotFoundLink({ href, icon: Icon, label }) {
  return (
    ...
    >
      ...
      <span className="font-semibold text-sm text-paper/85 group-hover:text-paper transition-colors">
        {label}
      </span>
      <ArrowUpRight className="w-4 h-4 ml-auto text-paper/30 group-hover:text-primary group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
    </Link>
  );
}
