    ...
    <>
      ...
      <section className="light-m relative section-pad overflow-hidden bg-canvas">
        <div className="container relative z-10 max-w-3xl">
          ...
          >
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-ink-30" />
            <input
              type="text"
              placeholder={ready ? t("faq:search") : ""}
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className="w-full pl-14 pr-5 py-4 rounded-2xl bg-canvas-1 border border-ink/[0.06] focus:border-primary/50 outline-none text-base text-ink placeholder:text-ink-30 transition-all"
            />
          </motion.div>
            ...
            {filtered.map((item, i) => {
              ...
              return (
                <motion.button
                  ...
                  className="group w-full text-left bg-canvas-1 hover:bg-canvas-2 border border-ink/[0.06] hover:border-primary/30 rounded-2xl transition-all overflow-hidden"
                >
                  <div className="flex items-start justify-between gap-4 p-5 lg:p-7">
                    <div className="flex items-start gap-4 flex-1">
                      <span className="text-xs font-mono text-ink-30 group-hover:text-primary transition-colors mt-1">
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mt-12 p-8 rounded-3xl bg-canvas-1 border border-ink/[0.06] text-center"
          >
